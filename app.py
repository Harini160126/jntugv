...
# Configure logging
if not app.debug:
    # Use stdout logging for production (Render, etc.)
...
@app.before_request
def init_db_before_first_request():
    """Initialize database tables before the first request."""
    if not hasattr(app, '_db_initialized'):
        with app.app_context():
            db.create_all()
            # Also run the init_db function to create default admin
            try:
                init_db()
            except Exception as e:
                app.logger.warning(f"init_db warning (may already exist): {e}")
